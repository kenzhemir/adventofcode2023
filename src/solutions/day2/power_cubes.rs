...
impl Pull {
    ...
    fn power(pull: Pull) -> u128 {
        pull.red * pull.blue * pull.green
    }
    fn from_str(input: &str) -> Pull {
        input
            .split(',')
            .fold(Pull::new(), |mut current_pull, single_ball_type| {
                let (amount_str, color) = single_ball_type.trim().split_once(' ').unwrap();
...
pub fn solve(input: &str) -> u128 {
    input
        .lines()
        .map(|line| {
            let (_, game_data) = line.trim().split_once(':').unwrap();
            game_data
                .split(';')
                .map(Pull::from_str)
                .reduce(Pull::max_merge)
                .unwrap_or(Pull::new())
        })
        .map(Pull::power)
        .sum()
...
#[cfg(test)]
mod tests {
    use crate::solutions::day2::power_cubes::solve;
